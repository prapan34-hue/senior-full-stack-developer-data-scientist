...
function Dashboard({ data, districts }) {
  const [selectedDistrict, setSelectedDistrict] = useState('all')
  const rawSeries = data.series?.length ? data.series : sampleSeries
  const series = useMemo(() => {
    if (selectedDistrict !== 'all') {
      return rawSeries.filter(row => row.district === selectedDistrict)
    }

    // Sample data is already one point per date. API data contains one point
    // per district, so aggregate it before drawing the province-wide line.
    if (!rawSeries.some(row => row.district)) return rawSeries
    const totalsByDate = rawSeries.reduce((totals, row) => {
      const key = row.record_date
      if (!totals[key]) totals[key] = { record_date: key, actual_cases: 0, predicted_cases: 0 }
      totals[key].actual_cases += Number(row.actual_cases || 0)
      totals[key].predicted_cases += Number(row.predicted_cases || 0)
      return totals
    }, {})
    return Object.values(totalsByDate).sort((a, b) => a.record_date.localeCompare(b.record_date))
  }, [rawSeries, selectedDistrict])
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-[1.55fr_1fr] gap-5">
      <section className="card p-6"><div className="flex justify-between gap-4 mb-5"><div><h2 className="font-semibold text-slate-800">ผู้ป่วยจริงเทียบค่าพยากรณ์</h2><p className="text-sm text-slate-400 mt-1">{selectedDistrict === 'all' ? 'ภาพรวมทั้งจังหวัดรายสัปดาห์' : `แนวโน้มรายสัปดาห์ • อ.${selectedDistrict}`}</p></div><label className="text-xs text-slate-500 min-w-44">เลือกพื้นที่<select aria-label="กรองกราฟตามอำเภอ" className="field mt-1 py-2 bg-white" value={selectedDistrict} onChange={event => setSelectedDistrict(event.target.value)}><option value="all">ทุกอำเภอ</option>{districts.map(district => <option key={district} value={district}>{district}</option>)}</select></label></div>
        {series.length ? <div className="h-72"><ResponsiveContainer><LineChart data={series}><CartesianGrid strokeDasharray="3 3" stroke="#e8f0f5"/><XAxis dataKey="record_date" tick={{fontSize:11}}/><YAxis tick={{fontSize:11}} allowDecimals={false}/><Tooltip labelFormatter={label => `วันที่ ${label}`}/><Legend/><Line type="monotone" dataKey="actual_cases" name="ผู้ป่วยจริง" stroke="#1479b8" strokeWidth={3} dot={{r:3}}/><Line type="monotone" dataKey="predicted_cases" name="ค่าพยากรณ์" stroke="#ef8b45" strokeWidth={3} strokeDasharray="6 4" dot={{r:3}}/></LineChart></ResponsiveContainer></div> : <div className="h-72 grid place-items-center rounded-xl bg-slate-50 text-sm text-slate-500">ยังไม่มีข้อมูลของอำเภอนี้ในช่วงเวลาที่เลือก</div>}
      </section>
